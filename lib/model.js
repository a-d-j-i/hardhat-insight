...
function processContractStorage(fullName, buildInfo) {
      ...
      if (node.nodeType === 'ContractDefinition') {
        ...
        travelAst(depth + 1, node, (d, n) => {
          if (n.nodeType === 'StructDefinition') {
            structs[n.id] = {
              name: n.name,
              slotFunc: (s) => processVars(s, n.members.map(x => x.typeName)),
              members: n.members.map(x => x.typeName.typeDescriptions.typeString + " " + x.name)
            };
          } else if (n.nodeType === 'EnumDefinition') {
            structs[n.id] = {
              name: n.name,
              slotFunc: (s) => 1, // enums take one uint?
              members: n.members.map(x => x.nodeType + " " + x.name)
            };
  ...
  for (const k in structs) {
    structs[k].slots = structs[k].slotFunc(structs);
  }

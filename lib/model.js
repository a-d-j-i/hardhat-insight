...
function processVars(vars, size, add) {
  let slot = 0;
  let offset = 0;
  let s = 0;
  for (const v of vars) {
    ...
    // Can be packed
    if (v.nodeType === "ElementaryTypeName") {
      const name = v.typeDescriptions.typeString;
      if (name === "address" || name.startsWith("uint")) {
        const bits = name === "address" ? 20 : parseInt(name.substring(4));
...
function sumSlots(vars) {
  return processVars(vars, (v) => {
    if (v.nodeType === "ArrayTypeName" && v.length) {
      return sumSlots([v.baseType]) * parseInt(v.length.value);
    }
    return 1;
...
module.exports = {processContractInsight, processContractStorage, sumSlots}

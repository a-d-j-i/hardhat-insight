...
function printContractStorage(
  ...
  const filtered = !!options.all ? table : Object.keys(table).reduce((acc, val) => {
    const t = table[val];
    const t0 = t[names[0]];
    if (t0 && t0.label && !names.every(name => t[name] && t[name].label === t0.label)) {
      acc[val] = t;
    }
    return acc;
  }, {});
  const pad = names.reduce((acc, val) => val.length > acc ? val.length : acc, 0) + 1;
  log(["slot", ...names.map(x => x.padEnd(pad))].join("\t"))
  for (const slot of Object.keys(filtered)) {
    const t = filtered[slot];
    log([slot, ...names.map(x => (t[x] ? t[x].label : "").padEnd(pad))].join("\t"))
  }
}
